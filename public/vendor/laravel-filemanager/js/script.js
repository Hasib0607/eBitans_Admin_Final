...
function use(items) {
    ...
    function useModalCallback(url) {
        if (!url) return;

        // Check if the modal is within an iframe or a popup window
        if (parent && parent.handleFileSelection) {
            // We are in an iframe, use the parent
            // parent.handleFileSelection(url); // Call the parent's method with the selected URL
        } else if (window.opener && window.opener.handleFileSelection) {
            // We are in a popup window, use window.opener
            // window.opener.handleFileSelection(url); // Call the opener's method
            window.close();
        } else {
            console.log("No parent window or window.opener found, and no handleFileSelection function");
        }
    ...
    if (useFileSucceeded) {
        useModalCallback(url);
    } else {
        console.log('window.opener not found');
        window.open(url); // Fallback: open the file directly
    }
}
