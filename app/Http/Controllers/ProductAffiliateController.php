...
class ProductAffiliateController extends Controller
{
    ...
    public function getWithdrawRequest(Request $request)
    {
        ...
        if ($firstSegment == 'api') {
            if (isset($request->user_id)) {
                $withdraws->where('info.user_id', $request->user_id);
            } else {
        ...
        $withdraws->where('users.store_id', $store_id);
        $withdraws = $withdraws->paginate(10);

        if ($firstSegment == 'api') {
            return response()->json($withdraws);
        }
        return view('admin.productAffiliateMarketing.withdraw.index', compact('withdraws', 'urls'));
    }
    ...
    public function createWithdrawRequest(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json(['status' => false, 'message' => 'Validation error', 'errors' => $validator->errors()->all()]);
        }
        ...
        if (!isset($info)) {
            return response()->json(['status' => false, 'message' => 'Affiliate User Info not found']);
            ...
            return response()->json(['status' => false, 'message' => 'Withdraw amount must be greater than minimum withdraw amount']);
        }
        ...
        $withdraw->save();
        if ($withdraw) {
            return response()->json(['status' => true, 'message' => 'Withdraw Request Successfully', 'data' => $withdraw]);
        }
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getAffiliateOrderDetails(Request $request)
    {
