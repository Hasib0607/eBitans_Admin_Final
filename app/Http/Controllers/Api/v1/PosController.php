...
//        $product = Product::convertCurrency($store_id)->find($id);
    ...
    public function addveritocart(Request $request)
    {
        ...
        $customeVolume = !empty($request->volume) && $request->volume != 0 ? $request->volume : null;
        $customePrice = !empty($request->price) && $request->price != 0 ? $request->price : null;
        $customeOrder = !empty($request->customeOrder) && $request->customeOrder === 1 ? true : false;

        $store_id = Branch::where("id", $bid)->value("store_id");

        if ($customeOrder) {
            if (is_null($customeVolume) || is_null($customePrice)) {
                return response()->json(['status' => false, 'message' => "Please input volume and price"]);
            }
        }
        ...
        if ($customeOrder) {
            ...
            $remainVolume = (float)$product->volume - (float)$totalVariantVolume;
            if ($remainVolume <= 0 || $remainVolume < $customeVolume) {
                return response()->json(['status' => false, 'message' => "Volume out of stock"]);
            }
        ...
        if (isset($exist) && !$customeOrder) {
            ...
            $catitem->variant_id = $customeOrder ? NULL : $id;
            $catitem->name = $product->name;
            $catitem->custome_order = $customeOrder ? 1 : null;
            $catitem->bid = $request->bid;
            $img = explode(',', $product->images);
            $catitem->image = $img[0];
            $catitem->quantity = 1;
            ...
            $catitem->volume = $customeOrder ? $customeVolume : $veriant->volume;
            $catitem->save();
        }

        return response()->json(['status' => true, 'message' => "Product added to cart successfully", "data" => $catitem]);
    }
    ...
    public function posorder(Request $request)
    {
        ...
        try {
            ...
            if (isset($user)) {
                ...
                $notificationData = [
                    "title" => "New walking customer register (" . ($user->name ?? '') . ") - " . formatDateWithTime($user->created_at),
                    "type" => "user_create",
                    "user_type" => "admin",
            ...
            foreach ($request->items as $key => $item) {
                ...
                $customeOrder = isset($item['custome_order']) && $item['custome_order'] == 1 ? true : false;
                ...
                if ($customeOrder) {
                    $totalVariantVolume = Veriant::where("pid", $item['product_id'])->sum(DB::raw('COALESCE(volume, 0)'));

                    $remainVolume = (float)$product->volume - (float)$totalVariantVolume;
                    if ($remainVolume <= 0 || $remainVolume < $item['volume']) {
                        DB::rollBack();
                        return response()->json(['status' => false, 'message' => "($product->name) out of stock"]);
                    }

                    $newProductVolume = number_format(((float)$product->volume - (float)$item['volume'] ?? 0), 2);
                    $newProductQuantity = number_format(((float)$product->quantity - (float)$item['quantity'] ?? 1), 2);
                    $product->volume = $newProductVolume;
                    ...
                    if (!is_null($variantId)) {
                        ...
                        if (isset($variant)) {
                            ...
                            if (isset($variant->color)) {
                                $productName .= " Color: $variant->color";
                            }
                            if (isset($variant->size)) {
                                $productName .= " Size: $variant->size";
                            }
                            $productName .= " " . $variant->volume ? $variant->volume . $variant->unit ?? "" : "";

                            if ($variant->volume <= 0 || $variant->volume < $item['volume']) {
                                DB::rollBack();
                                return response()->json(['status' => false, 'message' => "($productName) out of stock"]);
                            }
                        } else {
                            DB::rollBack();
                            return response()->json(['status' => false, 'message' => "Product variant not found"]);
                        }

                        $newVariantQuantity = number_format(((float)$variant->quantity - (float)$item['quantity'] ?? 1), 2);
