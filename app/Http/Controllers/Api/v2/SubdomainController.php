...
class SubdomainController extends Controller
{
            ...
            if (isset($store)) {
                switch ($section) {
                    ...
                    case 'new_arrival_products':
                        $newArrivalProducts = $this->getNewArrivalProductData($store->id);

                        return response()->json(['status' => true, 'message' => 'Success', 'data' => $newArrivalProducts]);
    ...
    public function newArrivalProducts($store)
    {
        try {
            if (empty($store) || is_null($store)) {
                return response()->json(['status' => false, 'message' => 'Store is required']);
            }

            $storeData = is_numeric($store)
                ? Store::where('id', $store)->where('expiry_date', '>=', Carbon::now())->first()
                : Store::where('url', $store)->where('expiry_date', '>=', Carbon::now())->first();

            if (!$storeData) {
                return response()->json(['status' => false, 'message' => 'Store not found!']);
            }

            $products = $this->getNewArrivalProductData($storeData->id);
    ...
    {
        $cacheKey = "store_{$storeId}_new_arrival_products";

        $products = Cache::remember($cacheKey, now()->addMinutes(5), function () use ($storeId) {
            return Product::convertCurrency($storeId)
                ->where('products.status', 'active')
                ->with([
                    'getBrand' => function ($query) {
                        $query->select('id', 'name');
        ...
        return $this->getProductResponse($products, $storeId);
    }
