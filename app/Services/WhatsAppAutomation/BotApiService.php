...
class BotApiService
{
    ...
    protected function webhookClient(string $secret = ''): PendingRequest
    {
        $baseUrl = rtrim((string) config('whatsapp_automation.bot_api_url'), '/');
    ...
    public function getLeadComments(string $sessionId, array $query = []): array
    {
        $response = $this->client()->get("/leads/{$sessionId}/comments", $query);
        return $response->throw()->json();
    }

    public function addLeadComment(string $sessionId, array $payload): array
    {
        $response = $this->client()->post("/leads/{$sessionId}/comments", $payload);
        return $response->throw()->json();
    }
    ...
    public function sendSalesChat(string $sessionId, string $userMessage): array
    {
        $response = $this->client()->post('/sales/chat', [
            'session_id' => $sessionId,
            'message' => $userMessage,
            'user_message' => $userMessage,
        ]);
    ...
    public function forwardGatewayWebhook(array $payload, string $secret = ''): array
    {
        $response = $this->webhookClient($secret)->post('/webhooks/whatsapp-gateway', $payload);
        return $response->throw()->json();
    }
    ...
    public function getSalesStageFunnelAnalytics(array $query = []): array
    {
        $response = $this->client()->get('/analytics/sales-stage-funnel', $query);
        return $response->throw()->json();
    }

    public function getSalesObjectionBreakdownAnalytics(array $query = []): array
    {
        $response = $this->client()->get('/analytics/sales-objection-breakdown', $query);
        return $response->throw()->json();
    }

    public function getSalesCtaPerformanceAnalytics(array $query = []): array
    {
        $response = $this->client()->get('/analytics/sales-cta-performance', $query);
        return $response->throw()->json();
    }

    public function getSalesOfferAnalytics(array $query = []): array
    {
        $response = $this->client()->get('/analytics/sales-offer-analytics', $query);
        return $response->throw()->json();
    }

    public function getFollowupTimingInsightsAnalytics(array $query = []): array
    {
        $response = $this->client()->get('/analytics/followup-timing-insights', $query);
        return $response->throw()->json();
    }
    ...
    public function getUnifiedLeadHistory(string $sessionId, array $query = []): array
    {
        $response = $this->client()->get("/leads/{$sessionId}/history", $query);
        return $response->throw()->json();
    }
    ...
    public function saveMessageFeedback(
        ...
        string $wrongIntentKey = '',
        string $correctedIntentKey = '',
        string $wrongTopicKey = '',
        string $correctedTopicKey = '',
        string $feedbackUseMode = ''
    ): array
    {
        $payload = [
            'verdict' => $verdict,
        ...
        if ($preferredReply !== '') {
            $payload['preferred_reply'] = $preferredReply;
        }

        if ($wrongIntentKey !== '') {
            $payload['wrong_intent_key'] = $wrongIntentKey;
        }

        if ($correctedIntentKey !== '') {
            $payload['corrected_intent_key'] = $correctedIntentKey;
        }

        if ($wrongTopicKey !== '') {
            $payload['wrong_topic_key'] = $wrongTopicKey;
        }

        if ($correctedTopicKey !== '') {
            $payload['corrected_topic_key'] = $correctedTopicKey;
        }

        if ($feedbackUseMode !== '') {
            $payload['feedback_use_mode'] = $feedbackUseMode;
        }

        $response = $this->client()->post("/messages/{$messageId}/feedback", $payload);
        return $response->throw()->json();
    }

    public function getReplyFeedbackList(array $query = []): array
    {
        $response = $this->client()->get('/feedback/replies', $query);
        return $response->throw()->json();
    }

    public function getReplyFeedbackLearningAnalytics(string $botType = 'sales'): array
    {
        $response = $this->client()->get('/analytics/reply-feedback-learning', [
            'bot_type' => $botType,
        ]);
        return $response->throw()->json();
    }

    public function updateReplyFeedbackApproval(int $feedbackId, array $payload): array
    {
        $response = $this->client()->post("/feedback/replies/{$feedbackId}/approval", $payload);
        return $response->throw()->json();
    }

    public function getSalesPromptVariant(): array
    {
        $response = $this->client()->get('/sales/prompt-variant');
        return $response->throw()->json();
    }

    public function updateSalesPromptVariant(string $variant): array
    {
        $response = $this->client()->post('/sales/prompt-variant', [
            'variant' => $variant,
        ]);
        return $response->throw()->json();
    }
