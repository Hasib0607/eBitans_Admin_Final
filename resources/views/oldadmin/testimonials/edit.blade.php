                ...
                <div class="col-lg-6" style="margin:0 auto;">
                    <div class="card mb-4">
                        <div class="card-header">
                            <h4>@if(Session::has('lang') && Session::get('lang')=='bn') এডিট প্রশংসাপত্র @else Edit Testimonials @endif </h4>
                        </div>
                        <div class="card-body">
                                <div class="mb-4">
                                    <label for="product_name" class="form-label">@if(Session::has('lang') && Session::get('lang')=='bn') ছবি @else Image @endif</label>
                                    <br>
                                    <img src="{{URL::to('/')}}/assets/images/testimonials/{{$data->image}}" alt="" width="250px" style="padding:10px;border:1px solid gray;margin-top:5px;margin-bottom:5px;">
                                    <br>
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" placeholder="Type here" value="{{$data->name}}" class="form-control" id="title" name="name">
                                    @error('name')
                                            <p class="text-danger" role="alert">{{$message}}</p>
                                    @enderror
                                </div>
                                <div class="mb-4">
                                    <label class="form-label">@if(Session::has('lang') && Session::get('lang')=='bn') পেশা @else Occupation @endif</label>
                                    <input type="text" placeholder="Type here" class="form-control" id="subtitle" name="occupation" value="{{$data->occupation}}">
                                    @error('occupation')
                                            <p class="text-danger" role="alert">{{$message}}</p>
                                    @enderror
                                </div>
                                <div class="mb-4">
                                    <label class="form-label">@if(Session::has('lang') && Session::get('lang')=='bn') প্রতিক্রিয়া @else Feedback @endif </label>
                                    <!--<input type="text" placeholder="Type here" class="form-control" id="link" name="feedback" value="{{$data->feedback}}">-->
                                    <textarea class="form-control" name="feedback" id="feedback" rows="7">{{$data->feedback}}</textarea>
                                    @error('feedback')
                                            <p class="text-danger" role="alert">{{$message}}</p>
                                    @enderror
                                </div>
                                <div class="mb-4">
                                    ...
                                    <input type="number" placeholder="Type here" class="form-control" id="position" name="position" value="{{$data->position}}">
                                    @error('position')
                                            <p class="text-danger" role="alert">{{$message}}</p>
                                    @enderror
                                </div>
                                <div class="mb-4 row">
                                    ...
                                    <div class="form-check form-switch is-filled" style="text-align:center;padding-top:14px;">
                                        <input class="form-check-input" type="checkbox" id="flexSwitchCheckChecked" name="status" @if($data->status=="active") checked="" @endif style="margin:0 auto;">
