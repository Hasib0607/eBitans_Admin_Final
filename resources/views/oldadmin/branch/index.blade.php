                ...
                <ol class="breadcrumb" style="background-color:transparent;color:#fff">
                    <li class="breadcrumb-item active">
                        <a href="{{URL::to('/')}}/branch">
                            <img src="{{URL::to('/')}}/img/icons/branch.png"> <br> <span class="nav-link-text ms-1">@if(Session::has('lang') && Session::get('lang')=='bn') ব্রাঞ্চ @else Branches @endif</span>
                        </a>
                    </li>
                </ol>
            </nav>
...
<div class="container-fluid mt-4" id="toplist">
    <div class="row">
        ...
        <div class="col-md-6">
             @if(isset($branch) && $branch=='1' || Auth::user()->type=='admin')
            <ul>
                @if(count($branchs) == $plan->branch)
                @else
                <li style="padding:0px;border:0px;"><a href="{{route('admin.branch.create')}}" style="display:block;border-radius:0px !important" class="btn btn-primary"> Create New</a></li>
                @endif
                
                <li style="padding:0px;border:0px;"><a data-href="/tasks" onclick="exportTasks(event.target);" style="display:block;border-radius:0px !important" class="btn btn-secondary">Excel</a></li>
            </ul>
            @else
            @endif
        </div>
    ...
    <div class="row mt-5 productlist">
        ...
        <span style="color:#fff">Total Branch add {{count($branchs) ?? "0"}}/{{$plan->branch}}</span>
    </div>
        </div>
        <div class="col-12">
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-2" style="padding-right:1px;">
                        <form id="submitform" method="post" action="{{route('admin.changebranchssstatus')}}">
                        @csrf
                ...
                <div class="table-responsive" id="desktoptable">
                    <table class="table table-striped" width="100%">
                        ...
                        <tbody>
                            ...
                            @if(Auth::user()->type=="staff")
                                    <?php
                                        $stfs=DB::table('staff')->where('id',$staff->id)->whereRaw('FIND_IN_SET("'.$branchss->id.'", pos)')->first();
                                    ?>
                            @endif
                            @if(isset($branch) && $branch=='1' || isset($stfs)  || Auth::user()->type=='admin')
                            <tr style="border-bottom:1px solid rgba(0, 0, 0, 0.125)">
                                <td><input type="checkbox" name="selectedid" value="{{$branchss->id}}" id="id" class="checkSingle"></td>
                                <td>{{$branchss->name}}</td>
                                <td>{{$branchss->email}}</td>
                                <td>{{$branchss->phone}}</td>
                                <?php
                                $customer=DB::table('customers')->where('id',$branchss->customer_id)->first();
                                ?>
                                ...
                                <td>
                                    ...
                                    @if(Auth::user()->type=='admin' || isset($stfs))
                                    <a href="{{route('admin.branch.pos',encrypt($branchss->id))}}" class="btn btn-success">Pos</a>
                                    @endif
                                     @if(isset($branch) && $branch=='1' || Auth::user()->type=='admin')
                                    &nbsp;&nbsp;
                                    <a href="{{route('admin.addproductbranch',$branchss->id)}}"><img src="{{asset('img/add-product.png')}}" width="25px" height="25px"</a>
                                    &nbsp;&nbsp;
                                    
                                    <a href="{{route('admin.editbranch',$branchss->id)}}"><img src="{{asset('img/edit.png')}}" width="20px" height="20px"></a>
                                    &nbsp;&nbsp;
                                    <a href="{{route('admin.deletebranch',$branchss->id)}}"  onclick="return confirm('Are you sure you want to delete this item?');"><img src="{{asset('img/delete.png')}}" width="25px" height="25px"</a>
                                    @endif
                                </td>
                            </tr>
                            @endif
                ...
                <div class="table-responsive" id="mobiletable">
                    <table class="table" width="100%">
                        ...
                        <tr class="mobilefirstrow">
                            <th width="10%">
                                <input type="checkbox" name="selectedid" value="{{$branchss->id}}" id="id" class="checkSingle">
                            </th>
