...
<div class="container-fluid mt-4" id="toplist">
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <ul>
                <li class="active"><a href="{{route('posplans.create')}}">Create New</a></li>
            </ul>
        </div>
    </div>
            ...
            <div class="card-header">
                <div class="row">
                    <div class="col-md-2" style="padding-right:1px;">
                        <form method="post" action="{{route('superadmin.changeposplansssstatus')}}">
                        @csrf
                ...
                <div class="table-responsive">
                    <table class="table" width="100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><input type="checkbox" name="selectedid" value="{{$plan->id}}" id="id" class="checkSingle"></td>
                                <td>{{$plan->name}}</td>
                                <td>{{$plan->price}}</td>
                                <td>{{$plan->branch}}</td>
                                <td>{{$plan->staff}}</td>
                                ...
                                <td>{{$plan->order}}</td>
                                <td>
                                    <input type="number" name="position" value="{{$plan->position}}" id="position" style="width:40%">
                                </td>
                                <td>
                                    <div class="form-check form-switch" style="text-align:center;">
                                        <input class="form-check-input" type="checkbox" id="flexSwitchCheckChecked" style="margin:0 auto;" @if($plan->status=='active') checked @endif>
                                        <label class="form-check-label" for="flexSwitchCheckChecked"></label>
                                    </div>
                                </td>
                                <td>
                                    <a href="{{route('editposplan',$plan->id)}}" class="btn btn-secondary">Edit</a>
                                    <a href="{{route('deleteposplan',$plan->id)}}" class="btn btn-danger">Delete</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
