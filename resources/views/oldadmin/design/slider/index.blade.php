...
<div class="container-fluid mt-4" id="toplist">
    @if(isset($slider) && $slider=='1' || Auth::user()->type=='admin')
    <div class="row">
        <div class="col-md-6">
            <h4>@if(Session::has('lang') && Session::get('lang')=='bn') সব স্লাইডার @else All Slider @endif</h4>
        </div>
        <div class="col-md-6">
            <ul>
                <li style="padding:0px;border:0px;"><a href="{{route('admin.design.slider.create')}}" class="btn btn-primary" style="display:block;border-radius:0px !important">@if(Session::has('lang') && Session::get('lang')=='bn') নতুন স্লাইডার যোগ করুন @else Add New Slider @endif</a></li>
                <li style="padding:0px;border:0px;"><a data-href="/sliderexport" onclick="exportTasks(event.target);" style="display:block;border-radius:0px !important" class="btn btn-secondary">@if(Session::has('lang') && Session::get('lang')=='bn') এক্সপোর্ট @else Excel @endif</a></li>
            </ul>
        </div>
    </div>
                    ...
                    <table class="table table-striped" id="taskfilterresult" width="100%">
                        <thead>
                            <tr>
                                ...
                                <th width="20%">@if(Session::has('lang') && Session::get('lang')=='bn') শিরোনাম @else Title @endif</th>
                                <th width="20%">@if(Session::has('lang') && Session::get('lang')=='bn') উপ শিরোনাম @else Sub Title @endif</th>
                        ...
                        <tbody>
                            ...
                            <tr style="border-bottom:1px solid rgba(0, 0, 0, 0.125)">
                                <td><input type="checkbox" name="selectedid" value="{{$slider->id}}" id="id" class="checkSingle"></td>
                                <td>
                                    <img src="{{URL::to('/')}}/assets/images/slider/{{$slider->image}}" class="zoom" alt="{{$slider->title}}" width="100px">
                                </td>
                                <td>{{$slider->title}}</td>
                                <td>{{$slider->subtitle}}</td>
                                <td>
                                    <input type="hidden" name="idss" id="id" value="{{$slider->id}}">
                                    <input type="number" value="{{$slider->position ?? '0'}}" name="position" id="position">
                                </td>
                                <td>
                                    <div class="form-check form-switch" style="text-align:center;">
                                        <input class="form-check-input switchstatus" type="checkbox" data-id="{{$slider->id}}" id="flexSwitchCheckChecked" @if($slider->status=="active") checked="" @endif style="margin:0 auto;">
                                        <label class="form-check-label" for="flexSwitchCheckChecked"></label>
                                    </div>
                                </td>
                                <td>
                                    <a href="{{route('admin.slider.edit',$slider->id)}}"><img src="{{asset('img/edit.png')}}" width="20px" height="20px"></a>
                                    &nbsp;&nbsp;
                                    <a href="{{route('admin.slider.delete',$slider->id)}}" onclick="return confirm('Are you sure you want to delete this item?');"><img src="{{asset('img/delete.png')}}" width="25px" height="25px"></a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                ...
                <div class="table-responsive" id="mobiletable">
                    <table class="table" width="100%">
                        ...
                        <tr class="mobilefirstrow">
                            <th width="10%">
                                <input type="checkbox" name="selectedid" value="{{$slider->id}}" id="id" class="checkSingle">
                            </th>
                            <th width="20%" style="color:#f1593a">
                                Image
                            </th>
                            <td width="60%" style="color:#f1593a">
                                <img src="{{URL::to('/')}}/assets/images/slider/{{$slider->image}}" class="zoom" alt="{{$slider->title}}" width="50px" height="50px">
                            </td>
                            <td width="10%">
                        ...
                        <tr class="cat{{$key}}" style="display:none">
                            ...
                            <td width="60%" style="display: flex;justify-content: center;align-items: center;">
                                <div class="form-check form-switch" style="text-align:center;">
                                    <input class="form-check-input switchstatus" type="checkbox" id="flexSwitchCheckChecked" data-id="{{$slider->id}}" style="margin:0 auto;" @if($slider->status=='active') checked @endif>
                                    <label class="form-check-label" for="flexSwitchCheckChecked"></label>
                                </div>
                            </td>
                            <td width="10%"></td>
