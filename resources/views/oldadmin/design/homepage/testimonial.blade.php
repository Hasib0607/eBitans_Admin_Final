    ...
    <div class="row">
        <div class="col-md-3 left-menu card card-body mt-4">
            <ul style="padding-left:0rem;">
                ...
                <li class="active" style="margin-bottom:10px;border-radius:10px;cursor:pointer"><a href="{{route('admin.design.homepage.testimonial')}}" style="display:block">@if(Session::has('lang') && Session::get('lang')=='bn') প্রশংসাপত্র @else Testimonial @endif</a></li>
        ...
        <div class="col-md-9 rightmenu mt-4">
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-6">
                                    <h4>@if(Session::has('lang') && Session::get('lang')=='bn') সমস্ত প্রশংসাপত্র @else All Testimonial @endif</h4>
                                </div>
                                <div class="col-md-3"></div>
                                <div class="col-md-3">
                                    <form action="{{route('admin.design.homepage.testimonialfilter')}}" method="get">
                ...
                @endif
                            <!--<form action="{{route('admin.savetestimonial')}}" method="post">-->
                                @csrf
                                <?php $header1=DB::table('designs')->where('store_id',$store_id)->where('testimonial','null')->first();  ?>
                                <div class="form-group">
                                    <input type="radio" id="Homes" class="changetesti" name="testimonial" @if(isset($header1)) checked @endif value="null">&nbsp;&nbsp;&nbsp;
                                    <label for="Homes"> None </label>  &nbsp;&nbsp;&nbsp;
                                </div>
                                @if(isset($design) && count($design)>0)
                                @foreach($design as $key=>$dsg)
                                <?php $header1=DB::table('designs')->where('store_id',$store_id)->where('testimonial',$dsg->value)->first();  ?>
                                <!-- Modal -->
                                ...
                                <div class="form-group">
                                    <input type="radio" id="Home{{$key}}" name="testimonial" class="changetesti" @if(isset($header1)) checked @endif value="{{$dsg->value}}">&nbsp;&nbsp;&nbsp;
...
<script>
    $(document).ready(function(){
        $(".changetesti").on("click",function(){
            $url="/changetesti";
            var value=$(this).val();
            console.log(value);
            $.get($url,{value:value}, function(data){
               console.log(data); 
               toastr.success('Testimonial Design Successfully', 'Success');
            });
        });
    });
</script>
