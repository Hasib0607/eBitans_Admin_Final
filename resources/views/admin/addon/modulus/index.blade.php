...
@section('content')
    @php
        ...
        /**
         * Trial plan detection
         * Plan ID 6 and 9 are treated as Trial plans in this view.
         * During trial we only allow a few specific modules to be toggled ON/OFF.
         */
        $isTrialPlan = !empty($plan_id) && in_array((int)$plan_id, [6, 9], true);

        /**
         * Trial-allowed module IDs:
         * 107 = Change of plan
         * 111 = Image Converter (Webp)
         * 114 = Variant
         */
...
@foreach ($modulus as $key => $item)
    ...
    @php
        /**
         * Per-module permission helper for this card.
         * - Not trial -> everything allowed
         * - Trial     -> allow only the whitelisted modules
         */
        $canUseInTrial = (!$isTrialPlan) || in_array((int)$item->id, $trialAllowedModules, true);
    @endphp
    ...
    <div class="col-sm-6 col-md-4 col-lg-3 mb-4">
        <div class="card">
            <div class="card-header">
                ...
                {{-- Checking price & payment status to decide: toggle OR buy modal --}}
                @if ($item->priceStatus == 1)

                    @if ($item->paymentStatus == 1)

                        {{-- Special case: Advance payment module (id=106) --}}
                        @if ((string) $item->id === '106')

                            {{-- If no payment gateway active, show disabled switch (forces them to enable gateways first) --}}
                            @if ($advanceHeaderSetting->bkash != 'active'
                                && $advanceHeaderSetting->merchant_bkash != 'active'
                                && $advanceHeaderSetting->merchant_nagad != 'active')

                                <div id="advanceButton"
                                     class="form-check form-switch is-filled mb-0 p-0 pt-2"
                                     style="float: right;">
                                    <input id="advanceCheckbox" class="form-check-input" type="checkbox">
                                </div>

                            @else
                                {{-- Toggle for advance module --}}
                                <div class="form-check form-switch is-filled mb-0 p-0 pt-2" style="float: right;">

                                    {{-- ✅ Trial guard: only whitelist modules can toggle in trial --}}
                                    @if(!$canUseInTrial)
                        ...
                        @else
                            ...
                            <div class="form-check form-switch is-filled mb-0 p-0 pt-2" style="float: right;">
                                ...
                                @else
                                    ...
                                    {{-- Special case: id=104 shows king icon modal beside toggle --}}
                                    @if ((string) $item->id === '104')
                                        <div class="form-check form-switch is-filled mb-0 p-0"
                    ...
                    @else
                        {{-- Payment required: show buy modal button --}}
                ...
                @else
                    {{-- Free modules toggle (except special case id=133) --}}
                    <div class="form-check form-switch is-filled mb-0 p-0 pt-2" style="float: right;">
            ...
            <div class="card-body text-center p-0" style="position: relative; overflow: hidden;">
                ...
                @else
                    <div class="text-block">
                        {{-- id=133 has external editor link always --}}
                        @if ((int) $item->id == 133)
                            <span>{!! $item->title !!} <a href="https://ieditor.ebitans.com" target="_blank">Click To Edit</a></span>
                        @else
                            <span>
                                {!! $item->title !!}
                                @if($item->config_status)
...
@push('scripts')
    ...
    <script>
        ...
        let isCheckboxChecked = $('#advanceCheckbox').prop('checked');
        ...
        $("#advanceButton").on("click", function () {
            ...
            Swal.fire({
                title: 'Please... Active (Bkash/Ebitans Nagad) Payment Method First!',
                icon: 'error',
                confirmButtonText: 'OK',
            }).then((result) => {
                if (result.isConfirmed) {
                    // Reset to the original state
                    $('#advanceCheckbox').prop('checked', isCheckboxChecked);
                }
            });

            // Alternatively, you could always uncheck it after the alert
            // $('#advanceCheckbox').prop('checked', false);
        });
    </script>
    ...
    const isTrialPlan = @json($isTrialPlan ?? false);
    const trialAllowedModules = @json($trialAllowedModules ?? []);

    $(".switchstatus").on("change", function (event) {

        const $switch = $(this);                 // ✅ keep reference
        const modulusId = parseInt($switch.data("value"));
        const storeId = parseInt($switch.data("id"));

        // ✅ trial restriction (frontend)
        if (isTrialPlan && !trialAllowedModules.includes(modulusId)) {
            event.preventDefault();
            $switch.prop("checked", !$switch.prop("checked")); // revert
            return Swal.fire("Warning!", "You are in trial Mode", "warning");
        }

        const url = "/modulus/change-status";

        $.get(url, { modulus_id: modulusId, id: storeId })
            .done(function (data) {

                if (data && data.status) {
                    return Swal.fire("Success!", data.message || "Status changed", "success")
                        .then(() => location.reload());
                }

                // ❌ backend rejected -> revert switch + show warning
                $switch.prop("checked", !$switch.prop("checked"));
                return Swal.fire("Warning!", (data && data.message) ? data.message : "Action denied", "warning");
            })
            .fail(function (xhr) {
                // ❌ request failed -> revert switch + show error
                $switch.prop("checked", !$switch.prop("checked"));
        ...
        const trialAlert = (event) => {
            ...
            Swal.fire({
    icon: 'warning',
    title: 'Limited Access – Trial Plan',
    html: `
        You are currently on a <strong>Trial Plan</strong>.<br><br>
        Only the following modules are available during the trial period:
        <ul style="text-align:left; margin-top:10px;">
            <li>Image Converter (WebP)</li>
            <li>Additional Designs</li>
            <li>Product Variant Module</li>
        </ul>
        Upgrade your subscription to unlock full access to all modules.
    `,
    ...
    if (result.isConfirmed) {
        window.location.href = "{{ route('payment.packages') }}";
        // যদি route না থাকে তাহলে ব্যবহার করো:
