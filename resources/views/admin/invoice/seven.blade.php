        ...
        <div class="invoice-description" style="padding: 0 30px;">
            <div class="invoice-left-top float-left">
                ...
                <div class="address" style="line-height: 8px;font-weight: 300;">
                    <p> Address: {{ $order->edited_address ?? $order->address ?? "Not Available" }} </p>
            ...
            @if ($orderitems->count() > 0)
                        ...
                        <tr>
                            ...
                            <td style="padding: 0.2rem;">
                                @if(isset($oitm->color_name))
                                    <strong>{{$oitm->color_name}}</strong>
                                @else
                                    @if (isset($oitm->color) && !empty($oitm->color))
                                        <span class="mr-3"
                                              style="display:inline-block; background: {{ $oitm->color }}; width:20px;border-radius: 50%;">{{ $oitm->name }}</span>
                                    @endif
                                @endif
                            </td>
                            <td style="padding: 0.2rem;">
                ...
                <div class="row float-right">
                    <div style="line-height: 10px;padding: 15px;">
                        <p {{ $order->subtotal <= 0 ? 'hidden' : '' }}><strong>Subtotal:</strong> {{$order->code}}
                            {{ $order->subtotal }}</p>
                        <p {{ $order->discount <= 0 ? 'hidden' : '' }}><strong>Discount:</strong> {{$order->code}}
                            {{ $order->discount }}</p>
                        <p {{ $order->tax <= 0 ? 'hidden' : '' }}>
                            <strong>Tax:</strong> {{$order->code}} {{ $order->tax }}
                        </p>
                        <p {{ $order->shipping <= 0 ? 'hidden' : '' }}><strong>Shipping
                                Charge:</strong> {{$order->code}}
                            {{ $order->shipping }}</p>
                        <p {{ $order->total <= 0 ? 'hidden' : '' }}><strong>Total Amount:</strong> {{$order->code}}
                            {{ $order->total }}</p>

                        @if (ModulusStatus($store->id, 106))
                            <p {{ $order->paid <= 0 ? 'hidden' : '' }}><strong>Paid Amount:</strong> {{$order->code}}
                                {{ $order->paid }}</p>
                            <p {{ $order->due <= 0 ? 'hidden' : '' }}><strong>Due Amount:</strong> {{$order->code}}
                                {{ $order->due }}</p>
                        @endif
