...
@section('content')
    <main class="main-content position-relative border-radius-lg">
        @include('admin.announcement.sub_category')
                                ...
                                <div class="row mb-4">
                                    <div class="col-md-12">
                                        ...
                                        <input type="text" placeholder="Type here" class="form-control"
                                               id="announcement"
                                               name="announcement"
                                               value="{{ $announcement->announcement ?? old('announcement') ?? "" }}"
                                               required>
                                        @error('announcement')
                            ...
                            <div class="row">
                                <div class="col-md-2" style="padding-right:1px;">
                                    <form id="submitform" method="post"
                                          action="{{ route('admin.announcement.status.change') }}">
                                        @csrf
                                ...
                                <table class="table table-striped" id="taskfilterresult" width="100%">
                                    ...
                                    @foreach ($announcements as $item)
                                        <tr style="border-bottom:1px solid rgba(0, 0, 0, 0.125)">
                                            ...
                                            <td>
                                                <a href="{{ route("admin.announcement.index", ["id" => $item->id]) }}">
                                                    <img src="{{ asset('img/edit.png') }}" width="20px"
                                                         height="20px">
                                                </a>
                                                <a style="margin-left: 3px" href="#"
                                                   onclick="deleteAnnouncement(event,{{ $item->id }})">
                                                    <img src="{{ asset('img/delete.png') }}" width="25px"
                                                         height="25px">
                                                </a>
                                                <form id="deleteAnnouncement{{ $item->id }}" method="post"
                                                      action="{{ route('admin.delete.announcement', ['id' => $item->id]) }}">
                                                    @csrf
    ...
    <script>
        $(document).ready(function () {
            $(".switchstatus").on("change", function () {
                ...
                const routeUrl = `{{ route('admin.single.announcement.status.change', ['id' => '__city_id__']) }}`;
