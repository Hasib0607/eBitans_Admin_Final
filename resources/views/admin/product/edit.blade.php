...
@push('styles')
    ...
    <style>
        ...
        #preOrderDetails .ck-editor__editable {
...
@section('content')
    ...
    <section class="container-fluid content-main pt-2">
        <form action="{{ route('admin.updateproduct', $product['id']) }}" method="post"
            ...
            @include('admin.product.share.product-input-field')
        </form>
    </section>
@endsection
...
@push('scripts')
    ...
    <script>
        ...
        CKEDITOR.instances['editor'].on('instanceReady', function () {
            const description = {!! json_encode(isset($product) ? $product['description'] : old('description')) !!};
    ...
    <script>
        ...
        $(document).ready(function () {
            $(document).on('click', '.imageUploadRemoveBtn', function (e) {
                ...
                // File-manager previews use native onclick only (no data-remove-url)
                if (!url) {
                    return;
                }
                e.preventDefault();
    ...
    <script>
        ...
        function CheckFileType(fileType) {
            const allowedTypes = ["image/jpeg", "image/png", "image/svg", "image/webp", "image/jpg", "image/gif"];
    ...
    <script>
        function getVariantDeleteId(button) {
            return $(button).data('variant-id') || $(button).closest('tr').find('input[name="attriid"]').val();
        }

        function deleteVariant(button, url) {
            var id = getVariantDeleteId(button);

            if (!id) {
                swal.fire('Error!', 'Variant id not found. Please reload and try again.', 'error');
                return;
            }
            ...
            $.get(url, {id: id})
                ...
                .fail(function () {
                    swal.fire('Error!', 'Variant delete failed. Please reload and try again.', 'error');
                });
        }

        $(document).on('click', '.deleteattri', function () {
            deleteVariant(this, '/deleteattribute');
        });

        $(document).on('click', '.deletesizeattri', function () {
            deleteVariant(this, '/deletesizeattribute');
        });

        $(document).on('click', '.deleteonlycolorattri', function () {
            deleteVariant(this, '/deleteonlycolorattribute');
        });

        $(document).on('click', '.deleteunitattri', function () {
            deleteVariant(this, '/deleteunitattribute');
        });
    ...
    @if (ModulusStatus($store_id, 114))
        <script>
            let colorSizeRowIndex = parseInt(document.getElementById('colorSizeRowIndex')?.value || '0', 10);

            // Color and size variant row add
            function addRow() {
                colorSizeRowIndex++;
                var i = colorSizeRowIndex;
                    ...
                    size += ` <div class="row">
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="input-group">
                                    <input type="hidden" class="form-control" onchange="variantImage(event)" accept="image/*" name="cs_Image[` + i + `][]" />
                                    <button type="button"
            ...
            function tiggerFileSelect(button) {
                const inputId = button.closest('.input-group')?.querySelector('input[type="hidden"]');

                if (inputId) {
                    standalonFileManagerModal(inputId, true);
                }
            }
    ...
    <script>
        ...
        function handleStockChange(value) {
            if (value === 'pre_order') {
                ...
                $('input[name="pre_order_note"]').attr('required', true);
            } else {
                $('#preOrderDetails').hide();
                $('input[name="pre_order_note"]').removeAttr('required');
            }
        }

        $(document).ready(function () {
            // Trigger change on load based on the selected radio
            const selectedValue = $('input[name="stock_status"]:checked').val();
            handleStockChange(selectedValue);

            // Attach change event using jQuery
            $('input[name="stock_status"]').on('change', function () {
                handleStockChange($(this).val());
            });
        });
    </script>
@endpush
