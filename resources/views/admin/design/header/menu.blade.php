            ...
            <div class="row">
                ...
                @endif
                            <form action="{{route('admin.saveheadermenu')}}" method="post">
                                ...
                                <div class="form-group">
                                    <?php
                                    $help=DB::table("menus")->where('store_id',$store_id)->where('url','help')->first();
                                    ?>
                                    <input type="checkbox" id="Blog" name="home[]" @if(isset($help)) checked @endif value="Blog">&nbsp;&nbsp;&nbsp;
                                    <label for="Shop"><input type="text" name="homename[]" class="menunamefield" value="{{$help->name ?? 'Help'}}"></label> &nbsp;&nbsp;&nbsp;
                                    <input type="hidden" name="url[]" value="help">
                                    <input type="hidden" name="menuselect[]" value="Blog">
                                    <label for=""><input type="number" name="homesort[]" class="menupositionfield" value="{{$help->sort ?? '1'}}"  style="width:50%"></label>
                                </div>
