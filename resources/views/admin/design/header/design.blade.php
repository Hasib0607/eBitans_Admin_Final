    ...
    <div class="row">
        <div class="col-md-3 left-menu card card-body mt-4">
            <ul style="padding-left:0rem;">
                <li class="active" style="margin-bottom:10px;border-radius:10px;cursor:pointer"><a href="{{route('admin.design.design')}}" style="display:block">@if(Session::has('lang') && Session::get('lang')=='bn') ডিজাইন @else Design @endif</a></li>
                <li  style="margin-bottom:10px;border-radius:10px;cursor:pointer"><a href="{{route('admin.design.header')}}" style="display:block">@if(Session::has('lang') && Session::get('lang')=='bn') মেনু @else Menu @endif</a></li>
            </ul>
        </div>
        <div class="col-md-9 rightmenu mt-4">
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-6">
                                    <h4>@if(Session::has('lang') && Session::get('lang')=='bn') সব ডিজাইন @else All Design @endif</h4>
                                </div>
                                <div class="col-md-3"></div>
                                <div class="col-md-3">
                                    <form action="{{route('admin.headerdesign.typefilter')}}" method="get">
                ...
                @endif
                            <form action="{{route('admin.saveheaderdesign')}}" method="post">
                                @csrf
                                <div class="d-flex flex-wrap align-items-center" style="min-height:50px;">
                                    <div class="form-group  px-3 d-flex flex-column flex-sm-row align-items-center" style="min-height:80px;">
                                        <label for="" class="pe-2">
                                            Background Color:
                                        </label>
                                        <?php $headercolor= DB::table('designs')->where('store_id',$store_id)->first(); ?>
                                        <input type="color" id="head" name="headercolor" value="{{$headercolor->header_color ?? '#fff'}}" style="width:150px;">
                                    </div>
                                    <div class="form-group px-3 d-flex flex-column flex-sm-row align-items-center" style="min-height:80px;">
                                        <label for="" class="pe-2">
                                            Text Color:
                                        ...
                                        <?php $headercolor= DB::table('designs')->where('store_id',$store_id)->first(); ?>
                                        <input type="color" id="head" name="textcolor" value="{{$headercolor->text_color ?? '#000'}}" style="width:150px;">
                                    </div>
                                ...
                                <?php $header1=DB::table('designs')->where('store_id',$store_id)->where('header',$dsg->value)->first();  ?>
                                <!-- Modal -->
                                ...
                                <div class="form-group">
                                    <input type="radio" id="Home{{$key}}" name="header" class="changeheader" @if(isset($header1)) checked @endif value="{{$dsg->value}}">&nbsp;&nbsp;&nbsp;
                                    <label for="Home{{$key}}"> {{ \Illuminate\Support\Str::limit($dsg->name, 22, $end='...') }} </label>  &nbsp;&nbsp;&nbsp;
    ...
    $(document).ready(function(){
        $(".changeheader").on("click",function(){
            ...
            $.get($url,{value:value}, function(data){
               console.log(data); 
               toastr.success('Header Design Successfully', 'Success');
            });
        });
    });
</script>
