    ...
    <div class="row">
        <div class="col-md-3 left-menu card card-body mt-4">
            <ul style="padding-left:0rem;">
                ...
                <li  style="margin-bottom:10px;border-radius:10px;cursor:pointer"><a href="{{route('admin.design.homepage.banner')}}" style="display:block">@if(Session::has('lang') && Session::get('lang')=='bn') ব্যানার @else Banner @endif</a></li>
                <li  style="margin-bottom:10px;border-radius:10px;cursor:pointer"><a href="{{route('admin.design.homepage.banner.bottom')}}" style="display:block">@if(Session::has('lang') && Session::get('lang')=='bn') ব্যানার বটম @else Banner Bottom @endif</a></li>
                ...
                <li class="active" style="margin-bottom:10px;border-radius:10px;cursor:pointer"><a href="{{route('admin.design.homepage.featureproduct')}}" style="display:block">@if(Session::has('lang') && Session::get('lang')=='bn') বৈশিষ্ট্য পণ্য @else Feature Product @endif</a></li>
        ...
        <div class="col-md-9 rightmenu mt-4">
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-6">
                                    <h4> @if(Session::has('lang') && Session::get('lang')=='bn') সমস্ত বৈশিষ্ট্য পণ্য @else All Feature Product @endif</h4>
                                </div>
                                <div class="col-md-3"></div>
                                <div class="col-md-3">
                                    <form action="{{route('admin.design.homepage.featureproductfilter')}}" method="get">
                ...
                @endif
                            <!--<form action="{{route('admin.design.homepage.featureproduct.save')}}" method="post">-->
                                @csrf
                                <?php $header1=DB::table('designs')->where('store_id',$store_id)->where('feature_product','null')->first();  ?>
                                <div class="form-group">
                                    <input type="radio" id="Homes" class="changefpro" name="feature_product" @if(isset($header1)) checked @endif value="null">&nbsp;&nbsp;&nbsp;
                                    <label for="Homes"> None </label>  &nbsp;&nbsp;&nbsp;
                                </div>
                                @if(isset($design) && count($design)>0)
                                @foreach($design as $key=>$dsg)
                                <?php $header1=DB::table('designs')->where('store_id',$store_id)->where('feature_product',$dsg->value)->first();  ?>
                                <!-- Modal -->
                                ...
                                <div class="form-group">
                                    <input type="radio" id="Home{{$key}}" name="feature_product" class="changefpro" @if(isset($header1)) checked @endif value="{{$dsg->value}}">&nbsp;&nbsp;&nbsp;
...
<script>
    $(document).ready(function(){
        $(".changefpro").on("click",function(){
            $url="/changefpro";
            var value=$(this).val();
            console.log(value);
            $.get($url,{value:value}, function(data){
               console.log(data);
               toastr.success('Feature Product Design Successfully', 'Success');
            });
        });
    });
</script>
