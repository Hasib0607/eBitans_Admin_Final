    ...
    <div class="row mt-5 productlist">
        ...
        <div class="card">
            ...
            @if (Session::has('success_message'))
                ...
                @endif
                <form action="{{route('savepopupimg')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group mt-3">
                        <img src="{{URL::to('/')}}/assets/images/setting/{{$st->image}}" style="width:200px;padding:10px;margin-bottom:10px;">
                        <input type="hidden" name="oldImage" value="{{ $st->image }}">
                        <input type="file" name="image" class="form-control" style="width:60%">

                    </div>
                    <div class="form-group mt-3">
                        <input type="checkbox" name="status"  id="status" value="1" {{ $st->status? 'checked':'' }}><label for="status">Status</label>
                    </div>
                    <div class="form-group mt-3">
