                    ...
                    <ul>
                        <li style="padding:0px;border:0px;">
                            <a href="{{ route('affiliate.withdraw.pending') }}"
                               class="btn btn-primary" style="display:block;border-radius:0px !important">Pending List
                                List</a>
                        </li>
                        <li style="padding:0px;border:0px;">
                            ...
                            <div class="table-responsive">
                                <table class="table" id="taskfilterresult" width="100%">
                                    ...
                                    <tr>
                                        ...
                                        <th width="5%">Previous Balance</th>
                                        <th width="5%">New Balance</th>
                                    ...
                                    @if (isset($data) && count($data) > 0)
                                        @foreach ($data as $key => $dm)
                                            <tr style="border-bottom:1px solid rgba(0, 0, 0, 0.125)">
                                                ...
                                                <td>{{ $dm->name }}</td>
                                                <td>{{ (int)$dm->prev_balance}}</td>
                                                <td>{{ (int)$dm->prev_balance - (int)$dm->withdraw_amount }}</td>
                                                <td>{{ $dm->withdraw_amount }}</td>

                                                <td>{{ date('d-m-Y H:m:s', strtotime($dm->created_at)) }}</td>
                                                {{--                                                <td>--}}
                                                {{--                                                    <a href="{{ route('affiliate.withdraw.status.change.pending', $dm->id) }}"--}}
