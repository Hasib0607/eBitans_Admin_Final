                                        ...
                                        @foreach ($data as $key => $dm)
                                            <tr style="border-bottom:1px solid rgba(0, 0, 0, 0.125)">
                                                ...
                                                @if($dm->status == 'Completed')
                                                    <td class="text-success fw-bolder">{{ $dm->referral_commission }}</td>
                                                @else
                                                    <td class="text-danger fw-bolder">{{ $dm->referral_commission }}</td>
                                                @endif
                                                <td>
                                                    <form class="d-flex justify-content-center align-items-center"
                                                          action="{{ URL::to('superadmin/affiliate/commission/rate') }}"
                                                          method="post" enctype="multipart/form-data">
                                                        @csrf
                                                        <input type="hidden" name="id" value="{{$dm->user_id}}">
                                                        <input type="number" style="width: 100px; height: 30px"
                                                               class="form-control" id="referral_commission"
                                                               name="referral_commission"
                                                               value="{{$dm->referral_commission}}">
                                                        <button
                                                            style="float:center;margin-right:5px;margin-left:5px; border:none">
                                                            <img src="{{ URL::to('/') }}/img/update.png" width="25px"
                                                                 height="25px">
                                                        </button>
                                                    </form>
                                                </td>
