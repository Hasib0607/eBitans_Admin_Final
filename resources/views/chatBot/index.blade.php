                ...
                <div class="col-md-6">
                    <ul>
                        <li style="padding:0px;border:0px;">
                            <a href="{{ route('chatBot.questions.create') }}" class="btn btn-primary"
                            ...
                            <div class="row">
                                <div class="col-md-2" style="padding-right:1px;">
                                    <form id="submitform" method="post"
                                          action="{{ route('chatBot.questions.action') }}">
                                        @csrf
                                                ...
                                                @if(count($item['questions']) > 0)
                                                    ...
                                                    @foreach($item['questions'] as $question)
                                                        <p style="font-size: 14px; margin: 0px">
                                                            {{ $i++ }}
                                                            . {{ Str::of($question['question'])->limit(30) }}
                                                        </p>
                                                    @endforeach
                                                @endif
                                            </td>
                                            <td style="text-align: left;">
                                                @if(count($item['answers']) > 0)
                                                    ...
                                                    @foreach($item['answers'] as $answer)
                                                        <p style="font-size: 14px; margin: 0px">
                                                            {{ $i++ }}
                                                            . {{ Str::of($answer['answer'])->limit(30) }}
                                                        </p>
                                                    @endforeach
                                                @endif
                                            </td>
                                            <td>{{ $item['type'] == 1 ? "Tech" : "Sales" }}
                                            </td>
                                            <td>{{ $item['lang'] == 1 ? "বাংলা" : "English" }}
                                            <td>
                                                <a href="{{ route('chatBot.questions.edit', $item['group_id']) }}">
                                                    <img src="{{ asset('img/edit.png') }}" width="20px"
                                                         height="20px">
                                                </a>
                                                &nbsp;&nbsp;
                                                <a href="{{ route('chatBot.group.delete', ['id' => $item['group_id']]) }}"
